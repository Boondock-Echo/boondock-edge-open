...
const TopBar = ({
  ...
  // Available timezone options (simplified list for demo)
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                ...
                <div 
                  className="h-1 w-12 ml-2 rounded-full opacity-80 transition-all duration-300"
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`relative group inline-flex items-center justify-center h-9 w-9 rounded-full ${commonStyles.buttonBackground} ${commonStyles.buttonHoverBackground} transition-all duration-300`}
      ...
      {isOpen && (
        <div 
          className={`fixed inset-0 ${isDarkMode ? 'bg-gray-900/80' : 'bg-gray-800/30'} backdrop-blur-md z-30 flex items-center justify-center transition-opacity duration-300 ${isModalClosing ? 'opacity-0' : 'opacity-100'}`}
        >
          <div 
            className={`${commonStyles.modalBackground} rounded-2xl w-full max-w-md mx-4 shadow-2xl relative border ${commonStyles.borderColor} overflow-hidden transition-transform duration-300 ${isModalClosing ? 'scale-95' : 'scale-100'}`}
          >
            <div 
            ...
            <div className={`relative flex items-center justify-between p-4 border-b ${commonStyles.borderColor}`}>
              <h3 className={`text-lg font-bold tracking-wide ${commonStyles.textColor} transition-colors duration-300`}>
                MESSAGES SETTINGS
              </h3>
              <button
                onClick={closeModal}
                className={`p-1.5 rounded-full ${isDarkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-gray-100 hover:bg-gray-200'} transition-colors duration-200`}
            ...
            <div className="relative p-6 space-y-6">
              <div className="space-y-2">
                ...
                <div className={`relative rounded-lg border ${commonStyles.borderColor} transition-colors duration-300`}>
                  <select
                    ...
                    style={{ 
                      borderColor: branding.brandColors.primary + '40',
                    }}
                  >
              ...
              <div className="space-y-3">
                {[
                  { label: 'TIMING DATA', icon: Clock, state: showTime, setState: setShowTime },
                  { label: 'TAG INFO', icon: Tag, state: showCar, setState: setShowCar },
                  { label: 'RADIO CHANNEL', icon: Radio, state: showChannel, setState: setShowChannel },
                  { label: 'PERSON', icon: User, state: showPerson, setState: setShowPerson },
                ].map(({ label, icon: Icon, state, setState }) => (
                  <div 
                    key={label} 
                    className={`flex items-center justify-between p-3 rounded-lg border ${state ? 'border-opacity-50' : 'border-opacity-20'} transition-all duration-300`}
                    style={{ 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`p-1.5 rounded-lg ${state ? '' : commonStyles.buttonBackground} transition-all duration-300`}
                        style={{ 
                          backgroundColor: state ? branding.brandColors.primary + '30' : '',
                        }}
                      >
                        <Icon 
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-all duration-300 ease-out focus:outline-none`}
                      style={{ backgroundColor: state ? branding.brandColors.primary : commonStyles.toggleInactive }}
                      aria-checked={state}
                      role="switch"
                    >
                      <span 
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-all duration-300 ease-out ${state ? 'translate-x-4' : 'translate-x-0.5'}`} 
                      />
                    </button>
                  </div>
                ))}
            ...
            <div className={`relative flex justify-end px-6 py-4 border-t ${commonStyles.borderColor} transition-colors duration-300`}>
              <button
                // onClick={applyChanges}
                className={`px-5 py-2 rounded-lg text-sm font-medium text-white shadow-md transition-all duration-300 hover:shadow-lg active:scale-95`}
                style={{ 
