...
const LoginPage = ({ isDarkMode, toggleTheme }) => {
  ...
  const [branding, setBranding] = useState({
    organizationName: 'US Police Department',
  ...
  useEffect(() => {
    const fetchBrandingData = async () => {
      try {
        ...
        setBranding({
          organizationName: data.organization_name || 'US Police Department',
        ...
        <div className="relative z-10 text-center space-y-4">
          {branding.assets.logo ? (
            <img
              ...
              className="w-24 h-24 md:w-32 md:h-32 object-contain mx-auto animate-glow"
            />
          ) : (
            <Satellite
